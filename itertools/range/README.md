...
Проблема в том, что в стандартной библиотеке (до C++20) нет сущности похожей на `range`, которую можно было использовать
в подобном контексте. Ну а раз так — надо ее реализовать самостоятельно!

**Дополнение:** в C++20 в стандартной библиотеке появилась
[сущность `std::ranges::iota_view`](https://en.cppreference.com/w/cpp/ranges/iota_view), которая решает эту задачу:
