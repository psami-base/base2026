...
целей в C++ есть механизм *исключений*. Исключение — это объект некоторого класса, который унаследован от стандартного
класса ошибки библиотеки C++. Если не все ясно сейчас, то это ok, если не все понятно года обучения — у вас проблемы.
...
1. Решение должно состоять из файлов `rational.hpp` (объявления) и `rational.cpp` (определения). Если для решения вам
...
3. Для вычисления НОД можете воспользоваться `std::gcd`.
